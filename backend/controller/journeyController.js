...
const startJourney = async (req, res) => {
  ...
  try {
    ...
    const isForwardDirection = startingIndex >= Math.min(sourceIndex, destinationIndex) && startingIndex <= Math.max(sourceIndex, destinationIndex);
    const isBackwardDirection = startingIndex <= Math.min(sourceIndex, destinationIndex) && startingIndex >= Math.max(sourceIndex, destinationIndex);
